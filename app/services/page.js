...
function ServiceSheet({ service, onSave, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', background: 'var(--bg-card)', borderRadius: '20px 20px 0 0', padding: '24px 20px 40px', fontFamily: "'DM Sans', sans-serif" }}>
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!profile || !['nail_artist', 'salon'].includes(profile.account_type)) {
