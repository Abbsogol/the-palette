    ...
    <div className="lq-bg-wine" style={{ minHeight: '100dvh', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', paddingBottom: 'calc(env(safe-area-inset-bottom) + 100px)' }}>
        ...
        <div style={{ padding: 'calc(env(safe-area-inset-top) + 16px) 20px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <h1 style={{ ...display(24), margin: 0 }}>Notifications</h1>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 20px', textAlign: 'center' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: PANEL, border: PANEL_BORDER, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
            ...
            <p style={{ ...ui(500, 15), marginBottom: '6px' }}>No notifications yet</p>
            <p style={ui(300, 13, WHITE60)}>When someone follows, likes, or comments, you&apos;ll see it here.</p>
          </div>
        ) : (
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    padding: '14px 12px', borderRadius: '14px', marginBottom: '4px',
                    textDecoration: 'none',
                    background: unread ? 'rgba(255,81,127,0.08)' : 'transparent',
                    border: unread ? '1px solid rgba(255,81,127,0.18)' : '1px solid transparent',
                  }}
                >
                  {/* Actor avatar */}
                  <div style={{
                    ...
                    border: PANEL_BORDER, position: 'relative',
                  }}>
                    {actor?.avatar_url
                      ? <img src={actor.avatar_url} alt={actor.display_name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      : <span style={ui(500, 16, ACCENT)}>{(actor?.display_name || '?')[0].toUpperCase()}</span>
                    }
                    {/* Type icon badge */}
                    <div style={{
                      position: 'absolute', bottom: -2, right: -2,
                      width: '18px', height: '18px', borderRadius: '50%',
                      background: ACCENT, border: '2px solid #260D14',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      {n.type === 'follow' && (
                        <svg width="9" height="9" viewBox="0 0 16 16" fill="none"><path d="M8 2v12M2 8h12" stroke="#260D14" strokeWidth="2.5" strokeLinecap="round"/></svg>
                      )}
                      {n.type === 'like' && (
                        <svg width="9" height="9" viewBox="0 0 16 16" fill="#260D14"><path d="M8 13.5C8 13.5 1.5 9.5 1.5 5.5C1.5 3.5 3 2 5 2C6.2 2 7.2 2.6 8 3.5C8.8 2.6 9.8 2 11 2C13 2 14.5 3.5 14.5 5.5C14.5 9.5 8 13.5 8 13.5Z"/></svg>
                      )}
                      {n.type === 'comment' && (
                        <svg width="9" height="9" viewBox="0 0 16 16" fill="none"><path d="M14 10a1 1 0 01-1 1H5l-3 3V3a1 1 0 011-1h10a1 1 0 011 1v7z" fill="#260D14"/></svg>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ ...ui(400, 13), margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                      {notifText(n)}
                    </p>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0 }}>
                    <span style={ui(300, 11, WHITE60)}>{timeAgo(n.created_at)}</span>
                    {unread && <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: ACCENT }} />}
                  </div>
                </Link>
              )
            })}
