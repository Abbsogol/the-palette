...
export async function transformDesignImage(rawBuffer, opts) {
  const { buffer } = await transformDesignImageMeta(rawBuffer, opts)
  return buffer
}

// Same pipeline, plus the output dimensions — THE one place image size is
// measured for designs.image_width/height. Insert call sites thread these
// two fields; none of them measure anything themselves.
export async function transformDesignImageMeta(rawBuffer, { maxDimension = MAX_DIMENSION } = {}) {
