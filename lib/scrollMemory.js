...
export function useScrollMemory(suffix, ready) {
  ...
  useEffect(() => {
    ...
    // NO write on cleanup: in the App Router the old page's effects clean
    // up AFTER the next route has rendered, so a cleanup write would read
    // the NEW page's scroll position and anchors under the OLD page's key —
    // exactly the clobber verification caught. The throttled writes during
    // scrolling already captured the resting position.
